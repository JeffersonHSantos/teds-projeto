    ...
    <x-ui.page-card>
        ...
        <form method="GET" action="{{ route('salas.index') }}">
            <div class="relative mb-4 rounded-md border border-gray-300 bg-gray-50 p-4">
                <div id="salas-filter-panel" class="fixed z-[9999] hidden w-[min(90vw,18rem)] max-h-[80vh] overflow-y-auto rounded-md border border-gray-200 bg-white p-2.5 shadow-2xl">
                    <div data-filter-section="salas" class="hidden space-y-2">
                        <label class="mb-2 flex items-center gap-2 font-medium text-gray-700">
                            <input type="checkbox" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" data-select-all="salas" {{ count($filtros['salas']) === $salaOptions->count() ? 'checked' : '' }}>
                            Todos
                        </label>
                        <div class="max-h-56 space-y-2 overflow-y-auto pr-1">
                            @foreach($salaOptions as $salaOption)
                                <label class="flex items-center gap-2 text-gray-700">
                                    <input type="checkbox" name="salas[]" value="{{ $salaOption->id }}" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" data-filter-item="salas" {{ in_array((string) $salaOption->id, $filtros['salas'], true) ? 'checked' : '' }}>
                                    <span>{{ $salaOption->nome }}</span>
                                </label>
                            @endforeach
                        </div>
                        ...
                        <div class="sticky bottom-0 mt-4 flex items-center gap-2 border-t border-gray-200 bg-white pt-3">
                            ...
                            <a href="{{ route('salas.index') }}" class="inline-flex rounded-md border border-gray-300 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 shadow-sm transition hover:bg-gray-50">
                                Limpar
                            </a>
                        </div>
                    </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                const filterPanel = document.getElementById('salas-filter-panel');
